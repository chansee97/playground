...
import type { ShortcutKeyMap } from "../keymap/types";
...
export interface MilkupConfig {
  ...
  /** 链接打开方式（默认使用 window.open 新窗口打开） */
  openLink?: (href: string) => void;
  /**
   * 自定义快捷键配置的提供者。
   *
   * 注入后内核不再去读桌面端约定的 localStorage；宿主改完设置后调用
   * `editor.invalidateShortcutConfig()` 让内核重新取一次。
   */
