...
/**
 * 插入图片：插入 markdown 原文并打上 image mark（渲染交给装饰层）
 */
export function insertImage(src: string, alt = "", title = ""): Command {
  return (state, dispatch) => {
    const markType = state.schema.marks.image;
    if (!markType) return false;

    if (dispatch) {
      const titlePart = title ? ` "${title}"` : "";
      const markdown = `![${alt}](${src}${titlePart})`;
      const { from, to } = state.selection;
      const tr = state.tr.insertText(markdown, from, to);
      tr.addMark(from, from + markdown.length, markType.create({ src, alt, title }));
      dispatch(tr.scrollIntoView());
    }
    return true;
  };
