...
/*
 * 内核不再通过副作用 import 引入样式（包括 KaTeX 的 CSS）：
 * 那会让内核无法脱离打包器使用。样式由宿主的样式管线负责，
 * Vue 层的 MarkdownEditor 已经引好 `milkup.css` + `katex/dist/katex.min.css`；
 * 直接使用内核的宿主需要自己引 `@playground/demo-markdown/style.css` 与 `katex/dist/katex.min.css`。
 */
...
import { createKeymapPlugin, createDynamicKeymapPlugin, createShortcutConfigSource } from "./keymap";
import type { ShortcutKeyMap } from "./keymap";
import { mapPositionByTextAnchor } from "./utils/text-position";
...
export class MilkupEditor implements IMilkupEditor {
   ...
   * 序列化结果缓存。
   *
   * 「每次按键全量序列化」本身是 v-model 契约的一部分（宿主需要新 Markdown），
   * 但同一次变更里往往会被要好几遍（`change` 事件一次、宿主回灌守卫比较一次、
   * 外部 `getMarkdown()` 若干次）。以 doc 实例为键缓存，doc 变了自然失效，
   * 于是每次变更只序列化一次。
   */
  private cachedMarkdown: { doc: Node; markdown: string } | null = null;
  /** 自定义快捷键配置的稳定快照（见 createShortcutConfigSource） */
  private shortcutConfig = createShortcutConfigSource(() => this.readShortcutConfig());
  ...
  constructor(container: HTMLElement, config: MilkupConfig = {}) {
    ...
    // 宿主的 localStorage 约定配置被其它上下文改写时，丢弃快捷键快照
    if (typeof window !== "undefined") {
      this.storageListener = (event) => {
        if (!event.key || event.key === "milkup-config") this.shortcutConfig.invalidate();
      };
  ...
  private createPlugins(): Plugin[] {
    const plugins: Plugin[] = [
      ...
      createDynamicKeymapPlugin(this.schema, () => this.shortcutConfig.get()),
   ...
   * 获取 Markdown 内容
   *
   * 结果按 doc 实例缓存：同一次变更里被问多少遍都只序列化一次。
   */
  getMarkdown(): string {
    const doc = this.view.state.doc;
    if (this.cachedMarkdown?.doc === doc) return this.cachedMarkdown.markdown;

    const markdown = this.serializer.serialize(doc);
    this.cachedMarkdown = { doc, markdown };
    return markdown;
  }
   ...
   * 设置 Markdown 内容（整篇替换）
   *
   * 整篇替换的 StepMap 会把选区塌缩掉，所以这里显式做两件事：
   *   1. 用文本锚点把光标映射到新文档的对应位置（而不是让它掉回文档开头）；
   *   2. 记录并补回光标在滚动容器里的偏移，避免视野跳走。
   */
  setMarkdown(content: string): void {
    const { doc } = this.parser.parse(content);
    const oldDoc = this.view.state.doc;
    const target = mapPositionByTextAnchor(oldDoc, this.view.state.selection.head, doc);
    const viewportOffset = getSelectionViewportOffset(this.view);

    const tr = this.view.state.tr
      .replaceWith(0, oldDoc.content.size, doc.content)
      .setMeta("addToHistory", false);
    tr.setSelection(Selection.near(tr.doc.resolve(Math.min(target, tr.doc.content.size)), 1));
    this.view.dispatch(tr);

    if (viewportOffset !== null) restoreSelectionViewportOffset(this.view, viewportOffset);
  }
  ...
  updateConfig(config: Partial<MilkupConfig>): void {
    ...
    // 配置更新视为「快捷键可能变了」：丢弃快照，下次按键重新读
    this.shortcutConfig.invalidate();
   ...
   * 丢弃自定义快捷键的缓存快照。
   *
   * 宿主改完自己的快捷键设置后调用；否则按键热路径会一直用旧的快照
   * （`shortcuts` 配置提供者返回新对象时同样需要调用它）。
   */
  invalidateShortcutConfig(): void {
    this.shortcutConfig.invalidate();
  }
  ...
  private initLinkHandler(): void {
    ...
    dom.addEventListener(
      ...
      (e: Event) => {
        ...
        if (href) {
          // 宿主可通过 openLink 覆盖默认行为（如桌面端交给系统浏览器打开）
          if (this.config.openLink) {
            this.config.openLink(href);
          } else {
  ...
  private getShortcutDisplay(actionId: ShortcutActionId): string {
    const customMap = this.shortcutConfig.get();
  ...
  private async insertImageFromFile(file: File): Promise<void> {
    // 获取图片粘贴方式；宿主没配置时回退 base64（不写磁盘、不依赖上传服务，任何环境都能跑通）
    const method: ImagePasteMethod =
      this.config.pasteConfig?.getImagePasteMethod?.() ?? "base64";
    ...
    // 图片是「mark + 真实文本」：插入 markdown 原文并打上 image mark，
    // 渲染交给装饰层。源码模式下同样插入原文（不渲染）。
    const alt = file.name;
    const markdown = `![${alt}](${src})`;
    const { from, to } = this.view.state.selection;
    const tr = this.view.state.tr.insertText(markdown, from, to);
    const mark = this.schema.marks.image?.create({ src, alt, title: "" });
    if (mark) {
      tr.addMark(from, from + markdown.length, mark);
    }
   ...
   * 读取用户自定义快捷键映射。
   *
   * 优先用宿主注入的 `config.shortcuts`（内核不再写死宿主约定）；没注入时才回落到
   * 桌面端约定：localStorage 里 `milkup-config` 的 `shortcuts` 字段。
   *
   * 注意：这个方法只在快照失效后被调用一次（见 `shortcutConfig`），
   * 不要把它直接挂到按键路径上。
   */
  private readShortcutConfig(): ShortcutKeyMap {
    if (this.config.shortcuts) return this.config.shortcuts() ?? {};
