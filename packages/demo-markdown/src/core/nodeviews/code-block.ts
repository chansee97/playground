...
export function detectDarkTheme(): boolean {
  ...
  // 约定 1：根元素上的 `theme-xxx-dark` 主题类名
  const themeClass = Array.from(htmlElement.classList).find((c) => c.startsWith("theme-"));
  if (themeClass) return themeClass.includes("dark");
...
function getMermaidThemeVariables(root?: Element): Record<string, string> {
  // 主题变量由编辑器容器提供，因此优先从传入节点（预览容器）读取，
  // 保证没有把变量挂到 :root 的宿主也能拿到正确取值。
  ...
  const baseTheme = EditorView.theme(
    {
      ...
      // 必须带兜底色：宿主没定义 --selected-background-color 时，
      // `var()` 解析失败会让 background-color 退成 transparent，
      // 于是代码块里划选没有任何反馈（看起来像选不中/复制不了）。
      "&.cm-focused .cm-selectionBackground, .cm-selectionBackground, .cm-content ::selection": {
        backgroundColor: "var(--selected-background-color, rgba(64, 128, 255, 0.3))",
      },
      ".cm-activeLine": {
        backgroundColor: "transparent",
      },
...
export class CodeBlockView implements NodeView {
  ...
  /** 语言扩展的加载代次：用于丢弃「加载完成时语言已经变了」的过期结果 */
  languageLoadToken = 0;
  destroyed = false;
  ...
  constructor(node: ProseMirrorNode, view: ProseMirrorView, getPos: () => number | undefined) {
    ...
    this.cm = new EditorView({
      state: CMEditorState.create({
        ...
        extensions: [
          ...
          // 语言扩展是动态 import 的，先挂空扩展，加载完成后 reconfigure
    ...
    // 语言扩展按需加载：拿到之后就挂上去
    this.applyLanguageExtension(normalizedLang);
  ...
  /**
   * 应用语言扩展（语言包按需 import，因此是异步的）。
   *
   * 竞态保护：加载期间语言可能又被切换，用代次计数丢弃过期结果；
   * 已经加载过的语言走同步快路径，不必等一个微任务。
   */
  private applyLanguageExtension(language: string): void {
    const cached = peekLanguageExtension(language);
    if (cached) {
  ...
  private setLanguage(language: string): void {
    ...
    // 更新 CodeMirror 语言扩展
    this.applyLanguageExtension(language);
    ...
    mermaidRenderQueue = mermaidRenderQueue.then(async () => {
      ...
      try {
        ...
        mermaid.default.initialize({
          ...
          darkMode: isDark,
          theme: "base",
          themeVariables: getMermaidThemeVariables(preview),
        });
  ...
  update(node: ProseMirrorNode): boolean {
    ...
    if (node.attrs.language !== prevLanguage) {
      this.applyLanguageExtension(node.attrs.language);
