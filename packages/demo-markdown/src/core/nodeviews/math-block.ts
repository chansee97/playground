 ...
 * 公式块实例按所属视图分组。
 *
 * 曾经是模块级 Set：`updateAllMathBlocks(view)` 会把该视图的选区灌给**所有**实例，
 * 于是同页第二个编辑器的公式块会跟着第一个编辑器的光标进出编辑态。
 */
const mathBlockViews = createViewGroupedRegistry<MathBlockView>();

export function updateAllMathBlocks(view: EditorView): void {
  const { from, to } = view.state.selection;
  for (const mathView of mathBlockViews.forOwner(view)) {
...
export class MathBlockView implements NodeView {
  ...
  destroy(): void {
    mathBlockViews.remove(this.view, this);
  }
}
